...
Автоматически определяет окружение (локальное/серверное)
"""
import sys
import os
...
# ========================================
# УНИВЕРСАЛЬНАЯ ЗАГРУЗКА ПЕРЕМЕННЫХ
# ========================================
# Пытаемся загрузить .env файл (для локальной разработки)
# На сервере этот файл отсутствует, и переменные берутся из системы
if os.path.exists('.env'):
    print("📁 Обнаружен файл .env - загрузка для локальной разработки", flush=True)
    load_dotenv()
else:
    print("☁️  Файл .env не найден - используются системные переменные (серверный режим)", flush=True)
